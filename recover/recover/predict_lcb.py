...
class LCBEvaluator(Evaluator):
    ...
    def load_dataset(self, path = "../language-confusion/test_sets", crossling=True, subset=None):
        if crossling:
            path = os.path.join(path, "crosslingual")
        else:
            path = os.path.join(path, "monolingual")
        ...
        if subset is not None:
            ...
            filter_languages =["er", "de", "es", "hi", "ja", "pt", "zh"] 
            df = df[df["language"].isin(filter_languages)]
...
def main():
    ...
    # Display args
    print(args)
    dataset_path="../language-confusion/test_sets"
    ...
    evaluator.load_dataset(path=dataset_path, crossling=args.crosslingual, subset=args.subset)
